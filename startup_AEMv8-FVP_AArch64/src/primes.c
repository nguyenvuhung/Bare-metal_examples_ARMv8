...
mutex prime_lock    __attribute__ ((aligned (64)));
mutex next_num_lock __attribute__ ((aligned (64)));
...
static unsigned int getNextNumber(void)
{
  ...
  _mutex_acquire(&next_num_lock);
  number = next_number;
  next_number = next_number + 2;
  _mutex_release(&next_num_lock);
...
{
  // Initialize mutexes
  _mutex_initialize(&prime_lock);
  _mutex_initialize(&next_num_lock);
