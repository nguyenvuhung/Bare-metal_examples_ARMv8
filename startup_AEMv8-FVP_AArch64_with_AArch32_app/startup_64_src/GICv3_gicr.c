...
typedef struct
{
  ...
  const volatile uint32_t GICR_TYPER[2];         // +0x0008 - RO - ???
        volatile uint32_t GICR_STATUSR;          // +0x0010 - RW - ???
        volatile uint32_t GICR_WAKER;            // +0x0014 - RW - ???
  const volatile uint32_t padding1[10];          // +0x0018 - RESERVED
        volatile uint64_t GICR_SETLPIR;          // +0x0040 - WO -
        volatile uint64_t GICR_CLRLPIR;          // +0x0048 - WO -
  const volatile uint32_t padding2[6];           // +0x0058 - RESERVED
        volatile uint32_t GICR_SEIR;             // +0x0068 - WO - ??? (Note: This was recently removed from the spec)
  const volatile uint32_t padding3;              // +0x006C - RESERVED
        volatile uint64_t GICR_PROPBASER;        // +0x0070 - RW - ???
        volatile uint64_t GICR_PENDBASER;        // +0x0078 - RW - ???
  const volatile uint32_t padding4[8];           // +0x0080 - RESERVED
        volatile uint64_t GICR_INVLPIR;          // +0x00A0 - WO - ???
  const volatile uint32_t padding5[2];           // +0x00A8 - RESERVED
        volatile uint64_t GICR_INVALLR;          // +0x00B0 - WO - ???
  const volatile uint32_t padding6[2];           // +0x00B8 - RESERVED
        volatile uint64_t GICR_SYNCR;            // +0x00C0 - WO - ???
  const volatile uint32_t padding7[2];           // +0x00C8 - RESERVED
  const volatile uint32_t padding8[12];          // +0x00D0 - RESERVED
        volatile uint64_t GICR_MOVLPIR;          // +0x0100 - WO - ??? (Note: This is an IMPDEF register)
  const volatile uint32_t padding9[2];           // +0x0108 - RESERVED
        volatile uint64_t GICR_MOVALLR;          // +0x0110 - WO - ???
  const volatile uint32_t padding10[2];          // +0x0118 - RESERVED (Note: This is an IMPDEF register)
